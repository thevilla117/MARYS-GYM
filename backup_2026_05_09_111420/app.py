...
with tab_hist:
    ...
    if not df_pagos.empty:
        ...
        for (ano, mes), group in grupos:
            ...
            with st.expander(f"💰 {nombre_mes} {ano}", expanded=esta_abierto):
                html = '''<div class="custom-table-container">
<table class="custom-table">
<thead>
<tr>
...
with tab_asist_hist:
    ...
    conn = sqlite3.connect('gym.db')
    df_asist_full = pd.read_sql_query("SELECT c.nombre, a.fecha as datetime_str FROM asistencia a JOIN clientes c ON a.cliente_id = c.id ORDER BY a.fecha DESC", conn)
    conn.close()
